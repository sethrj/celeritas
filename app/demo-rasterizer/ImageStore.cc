 ...
 */
ImageInterface ImageStore::device_interface()
{
    ImageInterface result;
 ...
 */
void ImageStore::image_to_host(SpanInt host_data) const
{
    REQUIRE(host_data.size() == image_.size());
    image_.copy_to_host(host_data);
}
