...
BetheHeitlerInteractor::sample_cos_theta(real_type kinetic_energy, Engine& rng)
{
    ...
    do
    {
        ...
        u = uu * BernoulliDistribution(0.25)(rng) ? real_type(1.6)
                                                  : real_type(1.6 / 3);
    } while (u > umax);
...
CELER_FUNCTION
real_type BetheHeitlerInteractor::screening_phiR(2)(real_type delta) const
{
    using R = real_type;
    return delta <= R(1.4)
               ? R(20.209) - R(1.930) * delta - R(0.086) * ipow<R(2)>(delta)
               : R(21.12) - R(4.184) * std::log(delta + R(0.952));
}
